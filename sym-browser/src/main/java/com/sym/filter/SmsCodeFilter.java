...
public class SmsCodeFilter extends OncePerRequestFilter {
    ...
    private final static Logger LOGGER = LoggerFactory.getLogger(SmsCodeFilter.class);
    ...
    public SmsCodeFilter(SymSecurityProperties symSecurityProperties, SymSignInFailedHandler symSignInFailedHandler){
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
        ...
        if( whetherToIntercept( request.getRequestURI() ) ){
            try{
                checkImageCode(request.getParameter(BrowserConstant.REQUEST_SMS_CODE),request);
...
//            flag = false;
//        }
        return "/sms/login".equals(requestUrl);
    }
    ...
    private void checkImageCode(String requestCode,HttpServletRequest request){
        ...
        String smsCode = (String)sessionStrategy.getAttribute(webRequest, BrowserConstant.SESSION_SMS_CODE);
...
//        }
        if( !requestCode.equalsIgnoreCase( smsCode ) ){
            sessionStrategy.removeAttribute(webRequest,BrowserConstant.SESSION_SMS_CODE);
